...
def train(model_hparam, pre_trained=None):
    train_x = torch.tensor(np.load("./Spectrogram/train_x_3.npy", allow_pickle = True), dtype=torch.float32)
    train_y = torch.tensor(np.load("./Spectrogram/train_y_3.npy", allow_pickle = True), dtype=torch.float32)
    ...
    for train_idx, val_idx in skf.split(train_x, train_y):
        ...
        if pre_trained != None:
            result_dict = torch.load(f"./model/result/{pre_trained}.pt")
        ...
        for epoch in range(epochs):
            ...
            if val_loss/len(val_loader) < best:
                ...
                torch.save({'hparam':model_hparam, 'weight':model.state_dict(), 'score' : best}, f'./model/result/{model_name}.pt')
...
if __name__ == "__main__":
    ...
    train(model_hparam, pre_trained='0404 1043')